...
const AVATAR_CONFIGS: Record<string, { scale: number; position: [number, number, number]; rotateY: number }> = {
  ...
  // Cantarella - needs 180 rotation, raised position for model origin
  ...
  // Eimi - new avatar
  "Eimi.vrm": { scale: 0.70, position: [0, 0.10, 0], rotateY: Math.PI },
  "/avatars/Eimi.vrm": { scale: 0.70, position: [0, 0.10, 0], rotateY: Math.PI },
};
...
function getMaxMouthOpen(avatarUrl: string): number {
  ...
  // Cantarella and Eimi can also handle larger mouth opening
...
export default function VRMAvatar({ url, emotion, audioUrl, lipSync, outfitVisibility = DEFAULT_OUTFIT }: VRMAvatarProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!lipSyncRef.current) {
        const maxMouthOpen = getMaxMouthOpen(url);
        console.log("📦 Creating new LipSyncManager with maxMouthOpen:", maxMouthOpen);
        lipSyncRef.current = new LipSyncManager(vrm, {
          smoothingFactor: 0.2,
